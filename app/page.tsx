...
export default function Entry(){
  ...
  // 寫本機
  function writeLocal(next: Partial<UI>){
    const merged = { ...ui, ...next }; setUI(merged)
    localStorage.setItem(LOCAL_KEY, JSON.stringify(merged))
  }

  // 真的一鍵升級：呼叫 /api/upgrade/start → 輪詢 /status → 完成導向 /fuceti
  async function startUpgrade(){
    setUpgrading(true); setProgress(8)
    try{
      const r = await fetch('/api/upgrade/start', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ jobType:'fuceti_init', plan:{} }) })
      ...
      const t = setInterval(async()=>{
        try{
          const s = await fetch(`/api/upgrade/status?jobId=${j.jobId}`).then(r=>r.json())
          if(s?.progress!=null) setProgress(s.progress)
          if(s?.status==='done'){
            clearInterval(t); setProgress(100)
            setTimeout(()=>{ setUpgrading(false); router.push('/fuceti') }, 500)
          }
          if(s?.status==='failed'){
            clearInterval(t); setUpgrading(false); alert('升級失敗，請稍後再試')
          }
        }catch{}
      }, 1500)
    }catch(e:any){
      setUpgrading(false)
      alert('升級啟動失敗：' + (e?.message || 'unknown'))
    }
  }
    ...
    <div className={`min-h-screen ${themeClass}`} style={{ ['--accent' as any]: ACCENT }}>
      ...
      <div className="w-full px-4 py-2 flex items-center justify-between border-b border-zinc-800/60 sticky top-0">
        ...
        <div className="flex items-center gap-2">
          <button onClick={startUpgrade} className="px-3 py-1 rounded-xl border border-zinc-700 hover:border-[var(--accent)]">一鍵升級</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4 p-4">
        <aside className="col-span-12 md:col-span-3 space-y-3 order-2 md:order-1">
          <Section title="快捷切換">
            <Row label="模型">
              <Select value={ui.model} onChange={(v)=>writeLocal({model: v as UI['model']})}
                options={[['deepseek-12b','12B'],['deepseek-30b','30B'],['gpt-4o','4o'],['gemini','Gemini'],['grok','Grok']]} />
            </Row>
            <Row label="語風">
              <Select value={ui.tone} onChange={(v)=>writeLocal({tone: v as UI['tone']})}
                options={[['neutral','一般'],['brief','精簡'],['playful','親切'],['strict','嚴謹']]} />
            </Row>
            <Row label="風格">
              <Select value={ui.style} onChange={(v)=>writeLocal({style: v as UI['style']})}
                options={[['gu-tech','gu-tech'],['plain','plain'],['掌門令','掌門令'],['工務札記','工務札記'],['溫柔說','溫柔說']]} />
            </Row>
            <Row label="主題">
              <Select value={ui.theme} onChange={(v)=>writeLocal({theme: v as UI['theme']})}
        ...
        <main className="col-span-12 md:col-span-9 space-y-4 order-1 md:order-2">
          <div className="rounded-2xl border border-zinc-800/60 p-4">
            ...
            <textarea className="w-full h-32 rounded-xl bg-transparent border border-zinc-800/60 p-3" placeholder="在這裡貼你的規劃…" />
            <div className="mt-3 flex gap-2 justify-end">
              <button className="px-4 py-2 rounded-xl border border-zinc-700">清除</button>
              <button className="px-4 py-2 rounded-xl border border-zinc-700">送出</button>
            </div>
          </div>
        </main>
      </div>
