...
function line(k,v,good){
  return '<div>'+k+'：<b class="'+(good?'ok':'bad')+'">'+v+'</b></div>'
}

async function checkAll(){
  const log = el('a1-log'); log.innerHTML='檢查中…'
  ...
  if(d){
    ...
    html += '<div>repo='+ (d?.repo?.owner||'?') +'/'+ (d?.repo?.repo||'?') + '</div>';
...
async function start(op){
  ...
  if(!r.ok || !r.js?.ok){ el('a23-log').innerHTML='失敗：'+(r.js?.error||('HTTP '+r.http)); return; }
...
el('btn-recheck').onclick=checkAll;
el('btn-gh').onclick=()=>start('gh_dispatch');
el('btn-vercel').onclick=()=>start('vercel_deploy');
checkAll();
</script>
</body></html>`;
