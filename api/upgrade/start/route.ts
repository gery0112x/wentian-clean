...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: job, error } = await sb.from('upgrade_jobs').insert({
      job_type: jobType, plan, status: 'running', progress: 10, log: ['workflow_dispatch']
    }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })

    // 觸發 GitHub Workflow
    const notifyURL = `${new URL(req.url).origin}/api/upgrade/notify`
    const payload = {
      ...
      inputs: {
        ...
        jobId: job.id,
        notifyURL,
        notifyToken: process.env.UPGRADER_WEBHOOK_TOKEN!,
